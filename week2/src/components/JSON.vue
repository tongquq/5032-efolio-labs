...
<template>
  ...
  <!-- Render success message when showMessage is true -->
  <p v-if="showMessage" class="message success">
    ✨ You're a Vue superstar! ✨
  </p>

   <!-- Render prompt message when showMessage is false -->
  ...
  <ul>
    <li
      ...
      :style="{
        backgroundColor: author.name === 'George Orwell' ? '#42b883' : '',
        color: author.name === 'George Orwell' ? 'white' : ''
      }"
    >
      {{ author.name }} （{{ author.birthYear }}）
    </li>
...
const orwell = computed(() =>
  authors.find(a => a.name === "George Orwell")
)

// Activity 5: Find author by ID
const austen = computed(() =>
  authors.find(a => a.id === 1)
)
</script>
